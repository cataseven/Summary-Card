...
class SummaryCard extends LitElement {
      ...
      case 'call-service':
        if (actionConfig.service) {
          ...
          if (actionConfig.service_data) {
            try {
              serviceData = typeof actionConfig.service_data === 'string' ? JSON.parse(actionConfig.service_data) : actionConfig.service_data;
            } catch (e) {
          ...
          if (actionConfig.target && actionConfig.target.entity_id) {
            ...
            if (typeof ids === 'string') {
              ids = ids.split(',').map(e => e.trim()).filter(e => e);
  ...
  async _evaluateTemplate(template, variables = {}) {
    if (!this.hass) {
      ...
      return `Error: Hass not available`;
    }
    try {
    ...
    for (const rule of cardConfig.styles) {
      ...
      if (conditionMet) {
        ...
        if (allTemplatesValid) {
          ...
          if (style.text) {
            style.text = await this._evaluateTemplate(style.text, templateVariables);
          }
          if (style.secondary_text) {
            style.secondary_text = await this._evaluateTemplate(style.secondary_text, templateVariables);
          }
      ...
      <div class="action-editor">
        ...
        ${action === 'call-service' ? html`
          ...
          <ha-textfield
              ...
              .configValue=${`cards.${cardIndex}.${actionKey}.service_data`}
              @input=${this._valueChanged}
