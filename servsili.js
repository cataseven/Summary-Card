...
// ... (DOMAIN_CONDITIONS ve DOMAIN_STATE_MAP sabitleri değişmeden kalır)
...
class SummaryCard extends LitElement {
  ...
  // ... (properties, constructor, connected/disconnectedCallback, getConfigElement, getStubConfig, setConfig, updated, _updateAllCardStyles, _debounce metodları değişmedi)
  ...
  // YENİ: Tıklama eylemlerini yöneten merkezi fonksiyon
  _performAction(actionConfig, cardConfig) {
    actionConfig = actionConfig || { action: 'more-info' }; // Varsayılan eylem

    switch (actionConfig.action) {
      case 'more-info':
        this._fireMoreInfo(cardConfig);
        break;
      case 'toggle':
        const entitiesToToggle = this._getEntities(cardConfig);
        const domain = cardConfig.domain;
        if (['light', 'switch', 'input_boolean', 'cover', 'lock'].includes(domain)) {
      ...
      case 'call-service':
        if (actionConfig.service) {
          const [serviceDomain, serviceName] = actionConfig.service.split('.');
          this.hass.callService(serviceDomain, serviceName, actionConfig.service_data);
        }
        break;
      case 'none':
        break;
  ...
  // DEĞİŞTİRİLDİ: _handleClick fonksiyonu yeniden adlandırıldı ve sadeleştirildi
  _fireMoreInfo(cardConfig) {
  ...
  _renderCard(cardConfig, index) {
    if (cardConfig.domain === 'clock') {
      ...
      return html`
        <div class="status-card" style="--icon-color: ${iconColor};" @click="${() => this._performAction(cardConfig.tap_action, cardConfig)}">
    ...
    // DEĞİŞTİRİLDİ: @click eylemi artık _performAction'ı çağırıyor
    return html`
      <div class="status-card ${!hasText ? 'icon-only' : ''}" style="--icon-color: ${style.color || "var(--primary-text-color)"};" @click="${() => this._performAction(cardConfig.tap_action, cardConfig)}">
        <div class="icon">
  ...
  // ... (_getEntities ve _computeCardStyleAsync metodları değişmedi)
...
class SummaryCardEditor extends LitElement {
  ...
  // YENİ: ha-service-control'den gelen değişiklikleri yöneten fonksiyon
  _serviceValueChanged(ev) {
    ev.stopPropagation();
    if (!this._config || !this.hass) return;
    const newConfig = JSON.parse(JSON.stringify(this._config));

    const path = ev.target.configValue.split('.');
    const cardIndex = parseInt(path[1], 10);
    const actionKey = path[2];

    const { service, data } = ev.detail.value;

    const card = newConfig.cards[cardIndex];
    if (!card[actionKey]) card[actionKey] = { action: 'call-service' };

    card[actionKey].service = service;
    card[actionKey].service_data = data;

    if (!service) {
      delete card[actionKey].service_data;
    }
          ...
          <div class="card-content">
            ...
            ` : html`
              ...
              <div class="interactions-container">
                ${this._renderActionEditor(card, cardIndex, 'tap_action', 'Tap Action')}
              </div>
    ...
    return html`
      <div class="action-editor">
        <ha-select
            label="${label}"
            .value=${action}
            .configValue=${`cards.${cardIndex}.${actionKey}.action`}
            @selected=${this._valueChanged}
            @closed=${(e) => e.stopPropagation()}
        >
          ${actions.map(act => html`<mwc-list-item .value=${act}>${actionLabels[act]}</mwc-list-item>`)}
        </ha-select>
        ...
        ${action === 'navigate' ? html`
          <ha-textfield
              ...
              .configValue=${`cards.${cardIndex}.${actionKey}.navigation_path`}
        ...
        ${action === 'url' ? html`
          <ha-textfield
              ...
              .configValue=${`cards.${cardIndex}.${actionKey}.url_path`}
              @input=${this._valueChanged}
          ></ha-textfield>
        ` : ''}
        ...
        ${action === 'call-service' ? html`
          <ha-service-control
              .hass=${this.hass}
              .value=${{ service: actionConfig.service, data: actionConfig.service_data }}
              .configValue=${`cards.${cardIndex}.${actionKey}`}
